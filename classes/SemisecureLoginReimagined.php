	...
	class SemisecureLoginReimagined {
		...
		function admin_print_scripts() {
			...
			switch ($pagenow) {
				...
				case 'user-edit.php':
					...
					break;
				case 'user-new.php':
					if (! $this->is_multisite()) // the password field isn't displayed here when multisite
						SemisecureLoginReimagined::enqueue_js();
					break;
			}
		}
		...
		function semisecure_activate() {
			...
			if (isset($successful) && $successful === FALSE) {
				$this->load_textdomain();
				$settings_page = ($this->is_multisite()) ? 'ms-admin.php?page=ms-semisecureloginreimagined' : 'options-general.php?page=semisecureloginreimagined';
		...
		function plugin_action_links($links) {
			if ($this->is_multisite()) {
				if (current_user_can(SEMISECURELOGIN_REIMAGINED__MANAGE_NETWORK_CAP)) {
					$this->load_textdomain();
					$link = '<a href="ms-admin.php?page=ms-semisecureloginreimagined">' . __('Network Settings', $this->text_domain) . '</a>';
					$links[] = $link;
				}
		...
		function get_option($shortname, $override=false) {
			if ($override || !SemisecureLoginReimagined::is_multisite()) {
				$get_func = 'get_option';
			}
			else {
			...
			switch($shortname) {
				...
				case 'encrypt_login':
					...
					if ($temp === FALSE) return 'yes'; // if not set al all, then default to "on"
					return ($temp) ? 'yes' : 'no';
				case 'rsa_keys':
		...
		/**
		 * Add support for this WP3.0 function w/o breaking backwards compatibility
		 * (can be used statically)
		 * 
		 * @return bool
		 */
		function is_multisite() {
			if (SEMISECURELOGIN_REIMAGINED__OVERRIDE_IS_MULTISITE)
				return false;
